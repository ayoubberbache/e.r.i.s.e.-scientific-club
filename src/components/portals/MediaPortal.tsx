...
import { 
  Camera, CheckSquare, Plus, CheckCircle2, Clock, 
...
export function MediaPortal({ onBackToAdmin, isSuperAdmin }: MediaPortalProps) {
  ...
  const loadTasks = async () => {
    ...
    try {
      const list = await fetchDepartmentTasks('Media');
      setTasks(list);
    } catch (err) {
      console.error('Error loading media tasks:', err);
    } finally {
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newTask = await createDepartmentTask('Media', {
  ...
  const handleMarkDone = async (taskId: string) => {
    try {
      await markDepartmentTaskDone('Media', taskId);
      setTasks((prev) =>
        prev.map((t) =>
  ...
  const handleDeleteTask = async (taskId: string) => {
    ...
    try {
      await deleteDepartmentTask('Media', taskId);
    ...
    <div className="space-y-5 text-slate-900 max-w-7xl mx-auto pb-10">
      ...
      <div className="bg-white border border-slate-200 p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-lg font-bold text-slate-900 mt-0.5">Media & Content Production</h1>
            <p className="text-xs text-slate-500">Head: {headConfig.name} • Audiovisual, Graphic Design & Social Coverage</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-white border border-slate-200 p-3.5">
          ...
          <div className="text-[11px] text-slate-500 mt-0.5">All media deliverables</div>
        </div>
        <div className="bg-white border border-slate-200 p-3.5">
          ...
          <div className="text-[11px] text-slate-500 mt-0.5">Assigned to team</div>
        </div>
        <div className="bg-white border border-slate-200 p-3.5">
          ...
          <div className="text-[11px] text-slate-500 mt-0.5">Verified deliverables</div>
        </div>
        <div className="bg-white border border-slate-200 p-3.5">
          <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Media Crew</div>
      ...
      {activeTab === 'tasks' && (
        <div className="bg-white border border-slate-200 p-5 space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
            <div>
              <h2 className="text-base font-bold text-slate-900">Creative & Coverage Tasks</h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Create photography, reel, video, or graphic design tasks. Click "Mark Done" to record completion.
              </p>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
            <div>
              <h2 className="text-base font-bold text-slate-900">
                Media Roster ({filteredMembers.length})
              </h2>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white border border-slate-300 p-4 sm:p-5 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <h3 className="font-bold text-slate-900 text-sm">Create Media Task</h3>
              <button
            ...
            <form onSubmit={handleCreateTask} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Event Teaser Reel & Sponsor Banner"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Specify dimensions (e.g. 1080x1920 9:16), cameras needed, software, deliverables..."
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-slate-900 text-xs focus:outline-none focus:bg-white"
                />
              </div>
